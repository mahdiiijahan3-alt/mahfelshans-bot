...
AUTO_POST_HOURS = max(
    1,
    int(os.getenv("AUTO_POST_HOURS", "2"))
)
...
def rules_text():
    ...
    return (
        ...
        "3) عضویت در کانال تلگرام، در صورت فعال بودن این شرط، هنگام بررسی برنده کنترل می‌شود.\n\n"

        "4) دنبال‌کردن اینستاگرام و یوتیوب از شرایط اعلام‌شده برای واجدشرایط‌بودن است.\n\n"

        "5) فعال‌شدن قرعه‌کشی اصلی پس از رسیدن هر سه شبکه به هدف تعیین‌شده انجام می‌شود.\n\n"

        "6) نتیجه قرعه‌کشی و شرایط هر جایزه فقط از مسیرهای رسمی محفل اعلام می‌شود."
    )
...
def home_text(user):
    ...
    return (
        f"🎉 <b>به محفل خوش‌شانس‌ها خوش آمدی، {first_name}!</b>\n\n"

        "اینجا شرکت در برنامه‌های محفل رایگان است.\n\n"

        "👥 با دعوت دوستان می‌توانی شانس‌های بیشتری به دست بیاوری.\n\n"

        "🎯 هدف بزرگ محفل: رسیدن شبکه به ۱۰۰٬۰۰۰\n\n"
...
def youtube_followers_from_api():
    ...
    if not YOUTUBE_API_KEY:
        raise RuntimeError(
            "YOUTUBE_API_KEY تنظیم نشده است."
        )

    if not YOUTUBE_CHANNEL_ID:
        raise RuntimeError(
            "YOUTUBE_CHANNEL_ID تنظیم نشده است."
        )
...
async def set_ig_followers(
    ...
    await update.message.reply_text(
        f"✅ تعداد اینستاگرام ثبت شد: {count:,}"
    )


async def set_tg_followers(
    update: Update,
    context: ContextTypes.DEFAULT_TYPE
):
    ...
    await update.message.reply_text(
        f"✅ تعداد تلگرام ثبت شد: {count:,}"
    )


async def set_yt_followers(
    update: Update,
    context: ContextTypes.DEFAULT_TYPE
):
...
async def draw_cmd(
    ...
    await update.message.reply_text(
        f"🏆 <b>برنده مسابقه #{campaign_id}</b>\n\n"
        f"شناسه تلگرام: <code>{winner}</code>\n"
...
async def postnow(
    ...
    if ok:
        ...
        await update.message.reply_text(
            ...
            "ادمین‌بودن ربات در کانال و دسترسی ارسال پیام را بررسی کن."
        )
...
async def eligibility_callback(
    ...
    text = (
        ...
        "راستی‌آزمایی خودکار اینستاگرام هنوز فعال نشده است.\n\n"

        "یوتیوب: شرط اعلام‌شده است.\n"
        "راستی‌آزمایی خودکار یوتیوب هنوز فعال نشده است.\n\n"

        "فعال‌شدن قرعه‌کشی اصلی شبکه: "
        f"{'✅' if active else '❌'}"
    )
...
async def referral_callback(
    ...
    text = (
        ...
        "هر دعوت معتبر، "
        "یک شانس اضافه برایت ثبت می‌کند."
    )
...
async def my_chances_callback(
    ...
    await query.edit_message_text(
        ...
        (
            ...
            f"<b>{chances}</b>\n\n"
            "👥 برای افزایش شانس، "
            "دوستانت را با لینک دعوت اختصاصی خودت وارد محفل کن."
        ),
